...
When I initially ran the code there were 26144 rows and 7 columns BUT there was a taxon order with -99999, indicating a lack of data. 
...
The average percent cover is a simple way to visualize the effect of treatment (continual, annual, control) and taxon order on percent cover. Sicne percent cover was measured for species over the same time frame, simplyfying the treatment effect by averaging the percent cover should be OK. 
...
A simple plot that shows the percent cover of different taxon orders. Percent cover exceeds 100 because cover is added across five different sites and three different treatments. So potentially 14-15 different percent covers are represented in one bar. 
...
Since the Fstat is less extreme than the calculated Fstar we fail to reject the null hypothesis that the between and within group variances are significantly different. Additionally, we also cannot reject the null hypothesis that the means of all the groups are the same. This is concerned by running the actual ANOVA, which returns a p value greater than 0.05. 
